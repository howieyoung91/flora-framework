...
import xyz.yanghaoyu.flora.framework.core.beans.factory.support.ApplicationContextAwareProcessor;
...
import xyz.yanghaoyu.flora.framework.core.context.ConfigurableApplicationContext;
import xyz.yanghaoyu.flora.framework.core.context.event.*;
import xyz.yanghaoyu.flora.framework.core.convert.support.DefaultConversionService;
...
import static xyz.yanghaoyu.flora.framework.constant.BuiltInBean.CONVERTER_FACTORY_BEAN;
...
public abstract class AbstractApplicationContext
    ...
    protected void preInstantiateSingletons() {
    ...
    public <T> T getBeanOfType(Class<T> type) throws BeansException {
        Iterator<T> it = getBeansOfType(type).values().iterator();
        if (it.hasNext()) {
            return it.next();
        }
        throw new BeansException("No bean instanceof [" + type.getName() + "]");
    }

    private void finishRefresh() {
        publishEvent(new ContextRefreshedEvent(this));
        LOGGER.trace("refreshed ApplicationContext");
    }
